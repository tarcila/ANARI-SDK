...
MdlSdrShaderNode* MdlSdrShaderNode::ParseSdrDiscoveryResult(const NdrNodeDiscoveryResult &discoveryResult) {
    ...
    auto module = mi::base::make_handle(registry->loadModule(discoveryResult.resolvedUri, transaction.get()));
    if (!module)
        return nullptr;
    ...
    fprintf(stderr, " subidentifier is %s\n", subIdentifier.c_str());

    auto functionDefinition = mi::base::make_handle(registry->getFunctionDefinition(module.get(),
        subIdentifier, transaction.get()));
    TfScoped releaseFunctionDefinition([&functionDefinition]() { functionDefinition.reset(); });
...
MdlFunctionSdrNode::MdlFunctionSdrNode(const NdrNodeDiscoveryResult& discoveryResult,
...
MdlFunctionSdrNode::MdlFunctionSdrNode(const NdrNodeDiscoveryResult& discoveryResult,
        const mi::neuraylib::IFunction_definition* functionDefinition, mi::neuraylib::ITransaction* transaction)
    : MdlFunctionSdrNode(discoveryResult, functionDefinition, transaction,
    ...
    const mi::neuraylib::Annotation_wrapper *annotations)
{
  ...
  if (auto index =
    ...
    metadata[SdrPropertyMetadata->Page] = std::string(group);
          ...
          annotations->get_annotation_index("::anno::in_group(string,string,bool)");
      index != mi::Size(-1)) {
    const char *group = {};
    annotations->get_annotation_param_value(index, 0, group);
    ...
    metadata[SdrPropertyMetadata->Page] = page;
          ...
          annotations->get_annotation_index("::anno::in_group(string,string,string,bool)");
      index != mi::Size(-1)) {
    const char *group = {};
    annotations->get_annotation_param_value(index, 0, group);
    auto page = std::string(group);
...
template<typename ValueType, typename TypeType = std::decay_t<decltype(ValueType::get_type())>>
SdrShaderPropertyUniquePtr MdlFunctionSdrNode::createScalarInputProperty(
    const std::string& name, const TypeType* scalarType, const mi::neuraylib::IExpression_constant* defaultValueExp,
        const mi::neuraylib::Annotation_wrapper* annotations
) {

  VtValue defaultValue;
  auto scalarValue = mi::base::make_handle(defaultValueExp->get_value<ValueType>());
...
SdrShaderPropertyUniquePtr MdlFunctionSdrNode::createTextureInputProperty(
  ...
  VtValue defaultValue;
  auto textureValue = mi::base::make_handle(defaultValueExp->get_value<const mi::neuraylib::IValue_texture>());
...
NdrPropertyUniquePtrVec MdlFunctionSdrNode::GetShaderProperties(const NdrNodeDiscoveryResult& discoveryResult,
    ...
    auto annotationList = mi::base::make_handle(functionDefinition->get_parameter_annotations());
 
    for( mi::Size index = 0; index < count; index++) {
        auto type = mi::base::make_handle(types->get_type(index)->skip_all_type_aliases());
        std::string name = functionDefinition->get_parameter_name(index);
        auto defaultValue = mi::base::make_handle(defaults->get_expression( name.c_str()));
        auto annotationBlock = mi::base::make_handle(annotationList->get_annotation_block(index));
        ...
        switch (type->get_kind()) {
            ...
            //             defaultValue->get_interface<const mi::neuraylib::IExpression_constant>(),
            //             &annotationWrapper
            //     ));
            //     break;
            // }
            case mi::neuraylib::IType::TK_TEXTURE: {
                properties.push_back(
                    createTextureInputProperty(name,
            ...
            default: {
                fprintf(stderr, "Skipping unsupported parameter %s of type %i\n",
                    name.c_str(), type->get_kind());
                break;
            }
        }
    }
...
NdrTokenMap MdlFunctionSdrNode::GetShaderMetadata(const NdrNodeDiscoveryResult& discoveryResult,
        const mi::neuraylib::IFunction_definition* functionDefinition, mi::neuraylib::ITransaction* transaction) {
        ...
        metadata[TfToken("subIdentifier")] = functionDefinition->get_mdl_simple_name();
        return metadata;
}

MdlMaterialSdrNode::MdlMaterialSdrNode(const NdrNodeDiscoveryResult& discoveryResult,
        const mi::neuraylib::IFunction_definition* functionDefinition, mi::neuraylib::ITransaction* transaction)
    : MdlFunctionSdrNode(
...
NdrPropertyUniquePtrVec MdlMaterialSdrNode::GetShaderProperties(const NdrNodeDiscoveryResult& discoveryResult,
        const mi::neuraylib::IFunction_definition* functionDefinition, mi::neuraylib::ITransaction* transaction) {

    auto properties = MdlFunctionSdrNode::GetShaderProperties(discoveryResult, functionDefinition, transaction);
    properties.emplace_back(new SdrShaderProperty(
            TfToken("out"),
            ...
            {
                // Terminal //Struct
                // { SdrPropertyMetadata->RenderType, "struct"s },
                { SdrPropertyMetadata->RenderType, "terminal surface"s },
                { TfToken("structType"), "material"s },
                { TfToken("symbol"), "material"s },
                // { SdrPropertyMetadata->Target, discoveryResult.resolvedUri },
                // { SdrPropertyMetadata->ImplementationName, functionDefinition->get_mdl_simple_name() },
            }, // metadata
            {}, //hints
            {} // options
        ));
...
NdrTokenMap MdlMaterialSdrNode::GetShaderMetadata(const NdrNodeDiscoveryResult& discoveryResult,
        const mi::neuraylib::IFunction_definition* functionDefinition, mi::neuraylib::ITransaction* transaction) {
    return MdlFunctionSdrNode::GetShaderMetadata(discoveryResult, functionDefinition, transaction);
}
